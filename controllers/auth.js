    ...
    User.findOne({}).exec((error,user) => {
        if (user) {
            ...
            sendEmailOnRegister
                ...
                return res.json({message:`Email has been sent to ${email},follow the instruction to complete your registeration !`})
              }
            )
                .catch(error => {
                    console.log('error :',error)
                return res.status(400).json({message:`Couldn't send email to ${email} !`})
              }
            )
        }
    })
            ...
            User.findOne({ email }).exec((error, user)=> {
                if (user) {
                    ...
                    user.save((error, user) => {
                        if (error) {
                            return res.status(400).json({ error: "Error saving user in Database. Try again later." });
                        } else {
...
exports.authMiddleware = (req, res, next) => {
    console.log("authMiddleware -> Checking user exists + saving user info in header")
